...
def test_matmul(dim1, dim2, dim3, dim4, funcs, dtype, req_grad: Tuple[bool, bool], transpose: Tuple[bool, bool]):
    ...
    for i in range(25):
        ...
        if funcs[0] in [torch.mm, torch.matmul]:
            ...
            target = torch.randn(size=(dim2, dim4), device="xpu", requires_grad=req_grad[1])
...
def test_dim3_igemm(seq_dim, hidden_dim, batch_dim):
    ...
    for i in range(25):
        A = torch.randint(-128, 127, size=(batch_dim, seq_dim, hidden_dim), device="xpu").to(torch.int8)
        B = torch.randint(-128, 127, size=(batch_dim, seq_dim, 1024), device="xpu").to(torch.int8)
...
def test_igemm(hidden_dim, batch_dim, transpose, seq_dim):
    ...
    for i in range(k):
        ...
        A = torch.randint(-128, 127, size=shapeA, device="xpu").to(torch.int8)
        B = torch.randint(-128, 127, size=shapeB, device="xpu").to(torch.int8)
        if not transpose[0] and not transpose[1]:
...
def test_bench_matmul(batch, seq, model, hidden):
    ...
    A = torch.randn(batch, seq, model, device="xpu").half()
    B = torch.empty(hidden, model, dtype=torch.float16, device="xpu")
    torch.nn.init.xavier_uniform_(B)
    ...
    linear8bit = bnb.nn.Linear8bitLt(model, hidden, False, False).xpu().half()
    linear8bit.eval()

    outliers = torch.randint(0, model, size=(5,)).xpu()
    A[:, :, outliers] = 8.0

    linearMixedBit = bnb.nn.Linear8bitLt(model, hidden, False, False, threshold=6.0).xpu().half()
    # linearMixedBit.eval()

    linear8bit_train = bnb.nn.Linear8bitLt(model, hidden, False).xpu().half()
    linear8bit_train_thresh = bnb.nn.Linear8bitLt(model, hidden, False, threshold=6.0).xpu().half()
