                          ...
                          const sycl::nd_item<3> &item_ct1,const sycl_la &tacc,const  sycl_dacc_uc &dacc_A,const sycl::accessor<T, 1> &dacc_out, const sycl_dacc_float &dacc_code, const sycl_dacc_float &dacc_absmax )
{
  ...
  using group_load_uc = dpct_::group::workgroup_load<NUM_PER_TH, dpct_::group::load_algorithm::BLOCK_LOAD_DIRECT, unsigned char,  unsigned char *, sycl::nd_item<3>>;
  
  using group_store = dpct_::group::workgroup_store<NUM_PER_TH*((DATA_TYPE > 0) ? 2 : 1), dpct_::group::store_algorithm::BLOCK_STORE_DIRECT, T,  T *, sycl::nd_item<3>>;
...
unsigned  char A[512];
float out[512];
for(int i=0;i<512;i++){code[i]=1.0f; absmax[i]=0.5f; A[i]=1;}
int blocksize=512;
int n=512;

dequantizeBlockwise<float, 0>(code, A, absmax, out, blocksize, n);
return 0;
