                        ...
                        const sycl_dacc_float &dacc_code, const sycl_dacc_float &dacc_absmax)
{
  ...
  using group_load = dpct_::group::workgroup_load<NUM_PER_TH, dpct_::group::load_algorithm::BLOCK_LOAD_DIRECT, T,  T *, sycl::nd_item<3>>;
  using group_load_float = dpct_::group::workgroup_load<NUM_PER_TH, dpct_::group::load_algorithm::BLOCK_LOAD_DIRECT, float,  float *, sycl::nd_item<3>>;  
  using group_store_uc = dpct_::group::workgroup_store<(DATA_TYPE > 0) ? NUM_PER_TH/2 : NUM_PER_TH, dpct_::group::store_algorithm::BLOCK_STORE_DIRECT, unsigned char,  unsigned char *, sycl::nd_item<3>>;  
...
quantizeBlockwise<float, 0, 0>(code, A, absmax, out, rand, rand_offset, blocksize, n);

quantizeBlockwise<float, 1, 1>(code, A, absmax, out, rand, rand_offset, blocksize, n);

quantizeBlockwise<float, 0, 1>(code, A, absmax, out, rand, rand_offset, blocksize, n);

quantizeBlockwise<float, 1, 0>(code, A, absmax, out, rand, rand_offset, blocksize, n);
